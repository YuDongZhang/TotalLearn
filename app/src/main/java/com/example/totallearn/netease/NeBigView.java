package com.example.totallearn.netease;
...
public class NeBigView extends View implements GestureDetector.OnGestureListener,
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        //这里注释的原因是第一次要做的,你要做的就是显示 竖向的 , 不做缩放
        // 确定加载图片的区域
//        mRect.left = 0;
//        mRect.top = 0;
//        mRect.right = mImageWidth;
        // 得到图片的宽度，就能根据view的宽度计算缩放因子,就是缩放的比例
//        mScale = mViewWidth/(float)mImageWidth;
//        mRect.bottom = (int)(mViewHeight/mScale); //屏幕底部的位置
    ...
    protected void onDraw(Canvas canvas) {
        ...
        //这里要加载的是图片要显示的部分 , 未缩放之前的 , 就是缩放之后刚好可以显示在屏幕上
    ...
    class ScaleGesture extends ScaleGestureDetector.SimpleOnScaleGestureListener{
        ...
        public boolean onScale(ScaleGestureDetector detector) {
            float scale = mScale;
            //这个缩放就是在 本来就放大了 所以要减去 1 , 再来缩放
            scale += detector.getScaleFactor() -1;
            if(scale <= originalScale){
                scale = originalScale; //最小缩放
            }else if(scale > originalScale*5){
                scale = originalScale*5;//最大缩放
            }
